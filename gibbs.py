...
def sampleZ(prob):
	#prob: an array of proportional probability
	s = random.random()
	total = sum(prob)
	...
	for t in range(0, iteration):
		...
		for i in range(0, len(trainingSet)):
			...
			for j in range(0, len(doc.content)):
				word = doc.content[j]
				probZ = [None for y in range(0, numTopic)]
				for k in range(0, numTopic):
					...
					if doc.x[j]==0:
						...
						if topic[k]==doc.z[j]:
							nwkg -= 1
							ntkg -= 1
						probZ[k] = ((nkd+alpha)/(ntd+alpha*numTopic))*((nwkg+beta)/(ntkg+beta*len(allVoc)))
					else:
						if doc.label==0:
							...
							if topic[k]==doc.z[j]:
								nwk0 -= 1
								ntk0 -= 1
							probZ[k] = ((nkd+alpha)/(ntd+alpha*numTopic))*((nwk0+beta)/(ntk0+beta*len(allVoc)))
						else:
							...
							if topic[k]==doc.z[j]:
								nwk1 -= 1
								ntk1 -= 1
							probZ[k] = ((nkd+alpha)/(ntd+alpha*numTopic))*((nwk1+beta)/(ntk1+beta*len(allVoc)))
				zNew = sampleZ(probZ)
				probX = [None for y in range(0, 2)]
				nkwX0 = trainingDicW[(word, doc.z[j])].nwkg
				ntkX0 = trainingDicT[doc.z[j]].ntkg
				probX[0] = (1-lam)*((nkwX0+beta)/(ntkX0+beta*len(allVoc)))
				if doc.label==0:
					nkwX1 = trainingDicW[(word, doc.z[j])].nwk0
					ntkX1 = trainingDicT[doc.z[j]].ntk0
					probX[1] = lam*((nkwX1+beta)/(ntkX1+beta*len(allVoc)))
				else:
					nkwX1 = trainingDicW[(word, doc.z[j])].nwk1
					ntkX1 = trainingDicT[doc.z[j]].ntk1
					probX[1] = lam*((nkwX1+beta)/(ntkX1+beta*len(allVoc)))		
				xNew = sampleX(probX)
			...
			for j in range(0, len(doc.content)):
				...
				for k in range(0, numTopic):
					...
					if topic[k]==doc.z[j]:
						nkd -= 1
					if doc.x[j]==0:
						probZ[k] = ((nkd+alpha)/(ntd+alpha*numTopic))*trainingPhi[(word, topic[k])]
					else:
						if doc.label==0:
							probZ[k] = ((nkd+alpha)/(ntd+alpha*numTopic))*trainingPhi0[(word, topic[k])]
						else:
							probZ[k] = ((nkd+alpha)/(ntd+alpha*numTopic))*trainingPhi1[(word, topic[k])]
				zNew = sampleZ(probZ)
				probX = [None for y in range(0, 2)]
				probX[0] = (1-lam)*trainingPhi[(word, topic[k])]
				if doc.label==0:
					probX[1] = lam*trainingPhi0[(word, topic[k])]
				else:
					probX[1] = lam*trainingPhi1[(word, topic[k])]	
				xNew = sampleX(probX)
				#update counts
		...
		for j in range(0, len(doc.content)):
			...
			for k in range(0, numTopic):
				...
				if topic[k]==doc.z[j]:
					nkd -= 1
				if doc.x[j]==0:
					probZ[k] = ((nkd+alpha)/(ntd+alpha*numTopic))*finalPhi[(word, topic[k])]
				else:
					if doc.label==0:
						probZ[k] = ((nkd+alpha)/(ntd+alpha*numTopic))*finalPhi0[(word, topic[k])]
					else:
						probZ[k] = ((nkd+alpha)/(ntd+alpha*numTopic))*finalPhi1[(word, topic[k])]
			zNew = sampleZ(probZ)
			probX = [None for y in range(0, 2)]
			probX[0] = (1-lam)*finalPhi[(word, topic[k])]
			if doc.label==0:
				nkwX1 = testingDicW[(word, doc.z[j])].nwk0
				ntkX1 = testingDicT[doc.z[j]].ntk0
				probX[1] = lam*finalPhi0[(word, topic[k])]
			else:
				nkwX1 = testingDicW[(word, doc.z[j])].nwk1
				ntkX1 = testingDicT[doc.z[j]].ntk1
				probX[1] = lam*trainingPhi1[(word, topic[k])]	
			xNew = sampleX(probX)
			#update counts
			testingDicDCopy[(doc, doc.z[j])] -= 1
